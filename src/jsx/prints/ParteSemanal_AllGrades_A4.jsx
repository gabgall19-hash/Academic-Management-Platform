...
export const handlePrintParteSemanal_AllGrades = (data, allSchedules) => {
  ...
  const coursesHTML = data.allCourses.map(course => {
    ...
    const scheduleData = allSchedules.find(s => s.course_id === course.id) || { grid_data: '[]' };
      ...
      <table class="attendance-table" style="width: 49%;">
        ...
        <thead>
          <tr class="date-row">
            ...
            <th class="col-day-date"></th><th class="col-day-date"></th><th class="col-day-date"></th><th class="col-day-date"></th><th class="col-day-date"></th>
          </tr>
          <tr>
            <th class="col-num">#</th><th class="col-name">Apellidos y Nombres</th>
            <th class="col-day">Lu</th><th class="col-day">Ma</th><th class="col-day">Mi</th><th class="col-day">Ju</th><th class="col-day">Vi</th>
          </tr>
        </thead>
        <tbody>
          ${Array(cols.length || 20).fill(0).map((_, i) => {
            const s = cols[i];
            return `<tr><td class="col-num">${startIdx + i}</td><td class="col-name">${s ? (s.apellido + ' ' + s.nombre).toUpperCase() : ''}</td><td></td><td></td><td></td><td></td><td></td></tr>`;
          }).join('')}
        </tbody>
      </table>
    `;
    ...
    return `
      <div class="printable-page">
        ...
        <div class="attendance-container">
          ${renderAttendanceTable(Array(leftCount).fill(0).map((_, i) => leftCol[i]), 1)}
          ${rightCount > 0 ? renderAttendanceTable(Array(rightCount).fill(0).map((_, i) => rightCol[i]), leftCount + 1) : ''}
        </div>
  ...
  win.document.write(`
    <html>
      <head>
        <title>Parte Semanal Institucional</title>
        <style>
      ...
      <body>
        <div class="no-print-toolbar">
          <button class="btn-print" onclick="window.print()">🖨️ IMPRIMIR TODOS LOS PARTES (${data.allCourses.length})</button>
        </div>
        ${coursesHTML}
      </body>
    </html>
